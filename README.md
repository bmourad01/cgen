...
4. **Pseudo**: "pseudo"-assembly, CFG-based representation with concrete, target-specific instructions. This
   is generated at instruction selection time. Maintains the stack slot abstraction up until register allocation.

**Structured** is optional, but can be useful for front-ends.

The compilation pipeline roughly follows this plan:

```
+---------------------------+
|  Structured IR (optional) | Input can be parsed from textual representation, constructed
|                           | in-memory, or reconstructed from a Virtual IR program.
+-------------+-------------+
              |
              v
+---------------------------+
|  Virtual IR (input, or    | Input can be parsed from textual representation, constructed
|  via destructuring)       | in-memory, or destructured from above.
+-------------+-------------+
              |
              v
+---------------------------+
...
+-------------+-------------+
              ...
              v
+---------------------------+
| Virtual IR (mid-end       | Control-flow simplifications, algebraic rewrites, constant
| optimizations)            | propagation, constant folding, GVN, GCM, LICM, RLE,
|                           | store-to-load forwarding, etc.
+-------------+-------------+
              |
              v
+---------------------------+
| Virtual ABI IR (ABI       | Remove aggregate types, implement the platform-specific rules
| lowering)                 | for parameter passing and returning values from functions via
|                           | registers/stack, etc.
+-------------+-------------+
              |
              v
+---------------------------+
| Virtual ABI IR (more      | CSE, RLE, store-to-load forwarding, etc. Intended for cleaning
| optimizations)            | up the code generated by the previous pass.
+-------------+-------------+
              |
              v
+---------------------------+
| Pseudo IR (instr.         | Implements a greedy, maximal-munch algorithm to match tree
| selection)                | patterns over a DAG of the function, producing sequences of
|                           | native instructions.
+-------------+-------------+
              |
              v
...
| allocation, stack layout) | and assigns concrete stack locations to slots and spilled
|                           | variables.
+-------------+-------------+
              |
              v
