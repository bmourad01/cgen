...
type t = {
  ...
  mutable pinned : Bitset.Id.t;           (* IDs that should not be rescheduled. *)
...
let set_pinned t id = t.pinned <- Bitset.Id.set t.pinned id
let is_pinned t id = Bitset.Id.mem t.pinned id
