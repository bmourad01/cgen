%{
    ...
    type swcase =
      | Case    of Cgen_utils.Bv.t * Type.imm * Structured.stmt
    ...
    let tag = Dict.register "structured-parser-env" (module Env)
      ...
      let+ cs = Context.List.map cs ~f:(function
          ...
          | Case (i, t', _) ->
            Context.failf
              ...
              Cgen_utils.Bv.pp i Type.pp_imm t' Type.pp_imm t ()) in
      `sw (i, t, cs)
    }
  | HLT { !!`hlt }
