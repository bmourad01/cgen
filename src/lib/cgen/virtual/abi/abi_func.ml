...
module T = struct
  type t = {
    name  : string;
    slots : Slot.t Rrb.t;
    blks  : Abi_blk.t Rrb.t;
    args  : (arg * Type.basic) Rrb.t;
    dict  : Dict.t;
  } [@@deriving bin_io, compare, equal, sexp]
end
...
let entry fn = Abi_blk.label @@ Rrb.get_exn fn.blks 0
...
let remove_blk_exn fn l =
  ...
  else {fn with blks = Rrb.filter fn.blks ~f:(fun b -> not @@ Abi_blk.has_label b l)}
...
let has_blk fn l = Rrb.exists fn.blks ~f:(fun b -> Abi_blk.has_label b l)
let find_blk fn l = Rrb.find fn.blks ~f:(fun b -> Abi_blk.has_label b l)

let next_blk fn l =
  match Rrb.find_index fn.blks ~f:(fun b -> Abi_blk.has_label b l) with
...
let pp_args ppf fn =
  ...
  pp_rrb pp_arg_ty sep ppf fn.args;
...
let pp ppf fn =
  ...
  let sep ppf = Format.fprintf ppf "@;" in
  Format.fprintf ppf "@[<v 0>%a@]@;}" (pp_rrb Abi_blk.pp sep) fn.blks
