(* Ported from BAP's `bitvec` package. *)
...
module Impl = struct
  ...
  *)
  ...
  let (!!) x m = int x m [@@inline] [@@ocaml.warning "-32"]
  ...
  let convert tname size convert max_signed max_unsigned x =
    if Z.compare x max_signed <= 0 then convert x
    else if Z.compare x max_unsigned <= 0
  ...
  let sexp_of_t x = Sexplib0.Sexp.Atom (to_string x)
  let t_of_sexp = function
    | Sexplib0.Sexp.Atom x -> of_string x
    | Sexplib0.Sexp.List _ ->
      invalid_arg "Bv.t_of_sexp: expected an atom, got a list"
  ...
  include struct
    ...
    include Bin_prot.Utils.Make_binable_with_uuid(struct
        ...
        type nonrec t = t
        let caller_identity =
          Bin_prot.Shape.Uuid.of_string "9f2b6d3a-1c4e-4b8a-9e7f-3d5a2c1b0e8f"
        let to_binable = to_binary
        let of_binable = of_binary
      end)
  end
end
