...
let create_candidate slots rs v = {
  var = v;
  slot = Map.find_exn slots v;
  range = Map.find_exn rs v;
  adj = Var.Set.empty;
  assigned = false;
}

let slot_sa x = Slot.(size x.slot, align x.slot)

let compat_size_align x y =
  let sx, ax = slot_sa x in
  let sy, ay = slot_sa y in
...
let is_adjacent x y = Var.(x.var <> y.var) && compat_range x y
...
let partition slots rs =
  ...
  | len ->
    ...
    Vec.iter vs ~f:(fun x ->
        ...
        Seq.filter ~f:(is_adjacent x) |>
        Seq.iter ~f:(fun y -> x.adj <- Set.add x.adj y.var));
    ...
    while not @@ Vec.is_empty vs do
      ...
      if not x.assigned then
        ...
        let g = Vec.fold_right vs ~init:[x] ~f:(fun y g ->
