...
module Rules = struct
  ...
  let rules = [
    ...
    (Obr & [x; y; y]) => (Ojmp & [y]);
    (* true ? x : y = x. *)
    (Osel  `i8 & [exp (Obool  true); x; y]) => x;
    (Osel `i16 & [exp (Obool  true); x; y]) => x;
    (Osel `i32 & [exp (Obool  true); x; y]) => x;
    (Osel `i64 & [exp (Obool  true); x; y]) => x;
    (Osel `f32 & [exp (Obool  true); x; y]) => x;
    (Osel `f64 & [exp (Obool  true); x; y]) => x;
    (* false ? x : y = y *)
    (Osel  `i8 & [exp (Obool false); x; y]) => y;
    (Osel `i16 & [exp (Obool false); x; y]) => y;
    (Osel `i32 & [exp (Obool false); x; y]) => y;
    (Osel `i64 & [exp (Obool false); x; y]) => y;
    (Osel `f32 & [exp (Obool false); x; y]) => y;
    (Osel `f64 & [exp (Obool false); x; y]) => y;
    (* x ? y : y = y. *)
    (Osel  `i8 & [x; y; y]) => y;
    (Osel `i16 & [x; y; y]) => y;
    (Osel `i32 & [x; y; y]) => y;
